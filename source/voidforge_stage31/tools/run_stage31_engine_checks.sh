...
TMP_DIR="${TMPDIR:-/tmp}/voidforge_stage31_checks_$$"
mkdir -p "$TMP_DIR"
...
for scene in \
  ...
  stage28_power_priority_smoke_test \
  stage29_ship_terminal_smoke_test \
  stage30_block_configuration_smoke_test \
  stage31_hand_drill_smoke_test; do
